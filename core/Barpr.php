	...
	class Barpr
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM barpr WHERE barprid='$arg'");
                ...
                if($res->num_rows > 0)
                {
                    ...
                    for($i = 0; $i < count($items); $i++)
                    {
                        if($items[$i] != "")
                        {
                            ...
                            if($it->Item != "")
                            {
                                $fi = new Baritem($this->subscriber);
                    ...
                    //$this->Order_reference = new Barpo($this->subscriber);
		...
		public function Save()
		{
            ...
            if($this->Reference == "")
            {
                ...
                while($db->query("SELECT refrence FROM barpr WHERE refrence='$ref'")->num_rows > 0)
                {
                    $ref = "pr-".Random::GenerateId(10);
                }
                $this->Reference = $ref;
			...
			$order_reference = addslashes(is_a($this->Order_reference, "Barpo") ? $this->Order_reference->Id : $this->Order_reference);
			...
			if($res = $db->query("SELECT barprid FROM barpr WHERE barprid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE barpr SET refrence='$refrence',note='$note',items='$items',fulfilled='$fulfilled',order_reference='$order_reference',user='$user',total='$total' WHERE barprid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT barprid FROM barpr WHERE barprid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO barpr(barprid,created,refrence,note,items,fulfilled,order_reference,user,total) VALUES ('$id','$created','$refrence','$note','$items','$fulfilled','$order_reference','$user','$total')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM barpr WHERE barprid='$id'");
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM barpr WHERE refrence LIKE '%$term%' OR note LIKE '%$term%' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Barpr($subscriber);
                $ret[$i]->Id = $row['barprid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='barprid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM barpr WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Barpr($subscriber);
                $ret[$i]->Id = $row['barprid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM barpr ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Barpr($subscriber);
                $ret[$i]->Id = $row['barprid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM barpr");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Barpr($subscriber);
                $ret[$i]->Id = $row['barprid'];
        ...
        public static function Fulfilled(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM barpr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND fulfilled=1 ORDER BY id DESC");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Barpr($subscriber);
                $ret[$i]->Id = $row['barprid'];
        ...
        public static function Pending(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM barpr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND (fulfilled=0 AND order_reference='') ORDER BY id DESC");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Barpr($subscriber);
                $ret[$i]->Id = $row['barprid'];
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT * FROM barpr WHERE fulfilled=1");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM barpr WHERE fulfilled=0 AND order_reference=''");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM barpr");
            $db->close();
            return $res->num_rows;
        }
        ...
        public function GenerateOrder($itList, $user)
        {
            if($this->Order_reference == "")
            {
                ...
                while(($db->query("SELECT order_reference FROM barpr WHERE order_reference='$orderRef'")->num_rows > 0))
                {
                    $orderRef = "po-".Random::GenerateId(16);
                }
        ...
        public static function ByReference(Subscriber $subscriber, $reference)
        {
            ...
            $res = $db->query("SELECT * FROM barpr WHERE order_reference='$reference'");
            if($res->num_rows > 0)
            {
                $row = $res->fetch_assoc();

                $ret = new Barpr($subscriber);
                $ret->Id = $row['barprid'];
