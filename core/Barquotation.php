<?php
	...
	// Feb, 13/2020
	// Building class for Barquotation
	*/
	...
	class Barquotation
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM barquotaion WHERE barquotaionid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['barquotaionid'];
		...
		public function Save()
		{
			...
			if($res = $db->query("SELECT barquotaionid FROM barquotaion WHERE barquotaionid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE barquotaion SET displayreference='$displayreference',sms='$sms',email='$email',associatedsuppliers='$associatedsuppliers',responded='$responded',responsecomplete='$responsecomplete',note='$note',suppliers='$suppliers',items='$items' WHERE barquotaionid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT barquotaionid FROM barquotaion WHERE barquotaionid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO barquotaion(barquotaionid,created,displayreference,sms,email,associatedsuppliers,responded,responsecomplete,note,suppliers,items) VALUES ('$id','$created','$displayreference','$sms','$email','$associatedsuppliers','$responded','$responsecomplete','$note','$suppliers','$items')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM barquotaion WHERE barquotaionid='$id'");
		}
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM barquotaion WHERE displayreference LIKE '%$term%' OR sms LIKE '%$term%' OR email LIKE '%$term%' OR associatedsuppliers LIKE '%$term%' OR responded LIKE '%$term%' OR responsecomplete LIKE '%$term%' OR note LIKE '%$term%' OR suppliers LIKE '%$term%' OR items LIKE '%$term%' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Barquotation($subscriber);
                $ret[$i]->Id = $row['barquotaionid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='barquotaionid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM barquotaion WHERE ".$field." ='$term' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Barquotation($subscriber);
                $ret[$i]->Id = $row['barquotaionid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM barquotaion ORDER BY ".$field." ".$order." ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Barquotation($subscriber);
                $ret[$i]->Id = $row['barquotaionid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM barquotaion ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Barquotation($subscriber);
				$ret[$i]->Id = $row['barquotaionid'];
