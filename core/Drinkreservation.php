<?php
	...
	// Feb, 25/2020
	// Building class for Drinkreservation
	*/

	class Drinkreservation
	{
		public $Id = "";
		public $Created = 0;
		public $Customer = "";
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM drinkreservation WHERE drinkreservationid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['drinkreservationid'];
		...
		public function Save()
		{
			...
			if($res = $db->query("SELECT drinkreservationid FROM drinkreservation WHERE drinkreservationid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE drinkreservation SET customer='$customer',orderday='$orderday',orderhour='$orderhour',ordermin='$ordermin',paid='$paid',invoice='$invoice',drink='$drink',price='$price',tax='$tax',discount='$discount',ordergmt='$ordergmt',schedule='$schedule',quantity='$quantity' WHERE drinkreservationid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT drinkreservationid FROM drinkreservation WHERE drinkreservationid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO drinkreservation(drinkreservationid,created,customer,orderday,orderhour,ordermin,paid,invoice,drink,price,tax,discount,ordergmt,schedule,quantity) VALUES ('$id','$created','$customer','$orderday','$orderhour','$ordermin','$paid','$invoice','$drink','$price','$tax','$discount','$ordergmt','$schedule','$quantity')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM drinkreservation WHERE drinkreservationid='$id'");
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='drinkreservationid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM drinkreservation WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Drinkreservation($subscriber);
                $ret[$i]->Id = $row['drinkreservationid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM drinkreservation ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Drinkreservation($subscriber);
                $ret[$i]->Id = $row['drinkreservationid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM drinkreservation");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Drinkreservation($subscriber);
				$ret[$i]->Id = $row['drinkreservationid'];
