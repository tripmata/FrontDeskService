	...
	class Kitchenpo
	{
        ...
        public function Initialize($arg=null)
		{
			if($arg != null)
			{
				...
				$res = $db->query("SELECT * FROM kitchenpo WHERE kitchenpoid='$arg'");

				if($res->num_rows > 0)
				{
					$row = $res->fetch_assoc();
				
					$this->Id = $row['kitchenpoid'];
                    ...
                    for($i = 0; $i < count($it); $i++)
                    {
                        ...
                        $tm = $itm->Item;
                        $itm->Item = new Kitchenitem($this->subscriber);
		...
		public function Save()
		{
			...
			if($res = $db->query("SELECT kitchenpoid FROM kitchenpo WHERE kitchenpoid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE kitchenpo SET supplier='$supplier',items='$items',user='$user',receiver='$receiver',total='$total',paidamount='$paidamount',note='$note',paid='$paid',received='$received',pr='$pr',receivedate='$receivedate',paydate='$paydate',creditnote='$creditnote' WHERE kitchenpoid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT kitchenpoid FROM kitchenpo WHERE kitchenpoid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO kitchenpo(kitchenpoid,created,supplier,items,user,receiver,total,paidamount,note,paid,received,pr,receivedate,paydate,creditnote) VALUES ('$id','$created','$supplier','$items','$user','$receiver','$total','$paidamount','$note','$paid','$received','$pr','$receivedate','$paydate','$creditnote')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM kitchenpo WHERE kitchenpoid='$id'");
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM kitchenpo WHERE supplier LIKE '%$term%' OR items LIKE '%$term%' OR user LIKE '%$term%' OR receiver LIKE '%$term%' OR total LIKE '%$term%' OR paidamount LIKE '%$term%' OR note LIKE '%$term%' OR paid LIKE '%$term%' OR received LIKE '%$term%' OR pr LIKE '%$term%' OR receivedate LIKE '%$term%' OR paydate LIKE '%$term%'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Kitchenpo($subscriber);
                $ret[$i]->Id = $row['kitchenpoid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='kitchenpoid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM kitchenpo WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Kitchenpo($subscriber);
                $ret[$i]->Id = $row['kitchenpoid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM kitchenpo ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Kitchenpo($subscriber);
                $ret[$i]->Id = $row['kitchenpoid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM kitchenpo");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Kitchenpo($subscriber);
				$ret[$i]->Id = $row['kitchenpoid'];
        ...
        public static function ByReference(Subscriber $subscriber, $refrence)
        {
            ...
            $res = $db->query("SELECT * FROM kitchenpo WHERE pr='$refrence'");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Kitchenpo($subscriber);
                $ret[$i]->Id = $row['kitchenpoid'];
                ...
                for($j = 0; $j < count($it); $j++)
                {
                    ...
                    $tm = $itm->Item;
                    $itm->Item = new Kitchenitem($subscriber);
                    $itm->Item->Initialize($tm);
