	...
	class Pastrypr
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM pastrypr WHERE pastryprid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['pastryprid'];
                    ...
                    for($i = 0; $i < count($items); $i++)
                    {
                        if($items[$i] != "")
                        {
                            ...
                            if($it->Item != "")
                            {
                                $fi = new Pastryitem($this->subscriber);
                    ...
                    //$this->Order_reference = new Pastrypo($this->subscriber);
		...
		public function Save()
		{
            ...
            if($this->Reference == "")
            {
                ...
                while($db->query("SELECT refrence FROM pastrypr WHERE refrence='$ref'")->num_rows > 0)
                {
                    $ref = "pr-".Random::GenerateId(10);
                }
                $this->Reference = $ref;
			...
			$order_reference = addslashes(is_a($this->Order_reference, "Pastrypo") ? $this->Order_reference->Id : $this->Order_reference);
			...
			if($res = $db->query("SELECT pastryprid FROM pastrypr WHERE pastryprid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE pastrypr SET refrence='$refrence',note='$note',items='$items',fulfilled='$fulfilled',order_reference='$order_reference',user='$user',total='$total' WHERE pastryprid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT pastryprid FROM pastrypr WHERE pastryprid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO pastrypr(pastryprid,created,refrence,note,items,fulfilled,order_reference,user,total) VALUES ('$id','$created','$refrence','$note','$items','$fulfilled','$order_reference','$user','$total')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM pastrypr WHERE pastryprid='$id'");
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM pastrypr WHERE refrence LIKE '%$term%' OR note LIKE '%$term%' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Pastrypr($subscriber);
                $ret[$i]->Id = $row['pastryprid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='pastryprid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM pastrypr WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Pastrypr($subscriber);
                $ret[$i]->Id = $row['pastryprid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM pastrypr ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Pastrypr($subscriber);
                $ret[$i]->Id = $row['pastryprid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM pastrypr");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Pastrypr($subscriber);
				$ret[$i]->Id = $row['pastryprid'];
        ...
        public static function Fulfilled(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM pastrypr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND fulfilled=1 ORDER By id DESC");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Pastrypr($subscriber);
                $ret[$i]->Id = $row['pastryprid'];
        ...
        public static function Pending(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM pastrypr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND (fulfilled=0 AND order_reference='') ORDER BY id DESC");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Pastrypr($subscriber);
                $ret[$i]->Id = $row['pastryprid'];
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM pastrypr WHERE fulfilled=1");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM pastrypr WHERE fulfilled=0 AND order_reference=''");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM pastrypr");
            $db->close();
            return $res->num_rows;
        }
        ...
        public function GenerateOrder($itList, $user)
        {
            if($this->Order_reference == "")
            {
                ...
                while(($db->query("SELECT order_reference FROM pastrypr WHERE order_reference='$orderRef'")->num_rows > 0))
                {
                    $orderRef = "po-".Random::GenerateId(16);
                }
                ...
                {
                    $po = new Pastrypo($this->subscriber);
        ...
        public static function ByReference(Subscriber $subscriber, $reference)
        {
            ...
            $res = $db->query("SELECT * FROM pastrypr WHERE order_reference='$reference'");
            if($res->num_rows > 0)
            {
                $row = $res->fetch_assoc();

                $ret = new Pastrypr($subscriber);
                $ret->Id = $row['pastryprid'];
