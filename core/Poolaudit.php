	...
	class Poolaudit
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM poolaudit WHERE poolauditid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['poolauditid'];
		...
		public function Save()
		{
			...
			if($res = $db->query("SELECT poolauditid FROM poolaudit WHERE poolauditid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE poolaudit SET title='$title',items='$items',surplus='$surplus',shortage='$shortage',surplustotal='$surplustotal',shortagetotal='$shortagetotal' WHERE poolauditid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT poolauditid FROM poolaudit WHERE poolauditid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO poolaudit(poolauditid,created,title,items,surplus,shortage,surplustotal,shortagetotal) VALUES ('$id','$created','$title','$items','$surplus','$shortage','$surplustotal','$shortagetotal')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM poolaudit WHERE poolauditid='$id'");
		}
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM poolaudit WHERE title LIKE '%$term%' OR items LIKE '%$term%' OR surplus LIKE '%$term%' OR shortage LIKE '%$term%' OR surplustotal LIKE '%$term%' OR shortagetotal LIKE '%$term%' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Poolaudit($subscriber);
                $ret[$i]->Id = $row['poolauditid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='poolauditid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM poolaudit WHERE ".$field." ='$term' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Poolaudit($subscriber);
                $ret[$i]->Id = $row['poolauditid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM poolaudit ORDER BY ".$field." ".$order." ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Poolaudit($subscriber);
                $ret[$i]->Id = $row['poolauditid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM poolaudit ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Poolaudit($subscriber);
				$ret[$i]->Id = $row['poolauditid'];
