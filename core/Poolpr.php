	...
	class Poolpr
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM poolpr WHERE poolprid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['poolprid'];
                    ...
                    for($i = 0; $i < count($items); $i++)
                    {
                        if($items[$i] != "")
                        {
                            ...
                            if($it->Item != "")
                            {
                                $fi = new Poolitem($this->subscriber);
                    ...
                    //$this->Order_reference = new Poolpo($this->subscriber);
		...
		public function Save()
		{
            ...
            if($this->Reference == "")
            {
                ...
                while($db->query("SELECT refrence FROM poolpr WHERE refrence='$ref'")->num_rows > 0)
                {
                    $ref = "pr-".Random::GenerateId(10);
                }
                $this->Reference = $ref;
			...
			$order_reference = addslashes(is_a($this->Order_reference, "Poolpo") ? $this->Order_reference->Id : $this->Order_reference);
			...
			if($res = $db->query("SELECT poolprid FROM poolpr WHERE poolprid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE poolpr SET refrence='$refrence',note='$note',items='$items',fulfilled='$fulfilled',order_reference='$order_reference',user='$user',total='$total' WHERE poolprid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT poolprid FROM poolpr WHERE poolprid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO poolpr(poolprid,created,refrence,note,items,fulfilled,order_reference,user,total) VALUES ('$id','$created','$refrence','$note','$items','$fulfilled','$order_reference','$user','$total')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM poolpr WHERE poolprid='$id'");
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM poolpr WHERE refrence LIKE '%$term%' OR note LIKE '%$term%' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Poolpr($subscriber);
                $ret[$i]->Id = $row['poolprid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='poolprid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM poolpr WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Poolpr($subscriber);
                $ret[$i]->Id = $row['poolprid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM poolpr ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Poolpr($subscriber);
                $ret[$i]->Id = $row['poolprid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM poolpr");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Poolpr($subscriber);
				$ret[$i]->Id = $row['poolprid'];
        ...
        public static function Fulfilled(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM poolpr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND fulfilled=1 ORDER BY id DESC");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Poolpr($subscriber);
                $ret[$i]->Id = $row['poolprid'];
        ...
        public static function Pending(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM poolpr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND (fulfilled=0 AND order_reference='') ORDER BY id DESC");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Poolpr($subscriber);
                $ret[$i]->Id = $row['poolprid'];
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM poolpr WHERE fulfilled=1");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM poolpr WHERE fulfilled=0 AND order_reference=''");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM poolpr");
            $db->close();
            return $res->num_rows;
        }
        ...
        public function GenerateOrder($itList, $user)
        {
            if($this->Order_reference == "")
            {
                ...
                while(($db->query("SELECT order_reference FROM poolpr WHERE order_reference='$orderRef'")->num_rows > 0))
                {
                    $orderRef = "po-".Random::GenerateId(16);
                }
        ...
        public static function ByReference(Subscriber $subscriber, $reference)
        {
            ...
            $res = $db->query("SELECT * FROM poolpr WHERE order_reference='$reference'");
            if($res->num_rows > 0)
            {
                $row = $res->fetch_assoc();

                $ret = new Poolpr($subscriber);
                $ret->Id = $row['poolprid'];
