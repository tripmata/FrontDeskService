	...
	class Roomaudit
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM roomaudit WHERE roomauditid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['roomauditid'];
		...
		public function Save()
		{
			...
			if($res = $db->query("SELECT roomauditid FROM roomaudit WHERE roomauditid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE roomaudit SET title='$title',items='$items',surplus='$surplus',shortage='$shortage',surplustotal='$surplustotal',shortagetotal='$shortagetotal' WHERE roomauditid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT roomauditid FROM roomaudit WHERE roomauditid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO roomaudit(roomauditid,created,title,items,surplus,shortage,surplustotal,shortagetotal) VALUES ('$id','$created','$title','$items','$surplus','$shortage','$surplustotal','$shortagetotal')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM roomaudit WHERE roomauditid='$id'");
		}
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM roomaudit WHERE title LIKE '%$term%' OR items LIKE '%$term%' OR surplus LIKE '%$term%' OR shortage LIKE '%$term%' OR surplustotal LIKE '%$term%' OR shortagetotal LIKE '%$term%'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Roomaudit($subscriber);
                $ret[$i]->Id = $row['roomauditid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='roomauditid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM roomaudit WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Roomaudit($subscriber);
                $ret[$i]->Id = $row['roomauditid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM roomaudit ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Roomaudit($subscriber);
                $ret[$i]->Id = $row['roomauditid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM roomaudit");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Roomaudit($subscriber);
				$ret[$i]->Id = $row['roomauditid'];
