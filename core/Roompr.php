	...
	class Roompr
	{
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM roompr WHERE roomprid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['roomprid'];
                    ...
                    for($i = 0; $i < count($items); $i++)
                    {
                        if($items[$i] != "")
                        {
                            ...
                            if($it->Item != "")
                            {
                                $fi = new Roomitem($this->subscriber);
                    ...
                    //$this->Order_reference = new Roompo($this->subscriber);
		...
		public function Save()
		{
            ...
            if($this->Reference == "")
            {
                ...
                while($db->query("SELECT refrence FROM roompr WHERE refrence='$ref'")->num_rows > 0)
                {
                    $ref = "pr-".Random::GenerateId(10);
                }
                $this->Reference = $ref;
			...
			$order_reference = addslashes(is_a($this->Order_reference, "Roompo") ? $this->Order_reference->Id : $this->Order_reference);
			...
			if($res = $db->query("SELECT roomprid FROM roompr WHERE roomprid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE roompr SET refrence='$refrence',note='$note',items='$items',fulfilled='$fulfilled',order_reference='$order_reference',user='$user',total='$total' WHERE roomprid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT roomprid FROM roompr WHERE roomprid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO roompr(roomprid,created,refrence,note,items,fulfilled,order_reference,user,total) VALUES ('$id','$created','$refrence','$note','$items','$fulfilled','$order_reference','$user','$total')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM roompr WHERE roomprid='$id'");
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM roompr WHERE refrence LIKE '%$term%' OR note LIKE '%$term%' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Roompr($subscriber);
                $ret[$i]->Id = $row['roomprid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='roomprid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM roompr WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Roompr($subscriber);
                $ret[$i]->Id = $row['roomprid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM roompr ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Roompr($subscriber);
                $ret[$i]->Id = $row['roomprid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM roompr");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Roompr($subscriber);
				$ret[$i]->Id = $row['roomprid'];
        ...
        public static function Fulfilled(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM roompr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND fulfilled=1 ORDER BY id DESC");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Roompr($subscriber);
                $ret[$i]->Id = $row['roomprid'];
        ...
        public static function Pending(Subscriber $subscriber, $term='')
        {
            ...
            $res = $db->query("SELECT * FROM roompr WHERE (refrence LIKE '%$term%' OR note LIKE '%$term%') AND (fulfilled=0 AND order_reference='') ORDER BY id DESC ");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Roompr($subscriber);
                $ret[$i]->Id = $row['roomprid'];
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM roompr WHERE fulfilled=1");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM roompr WHERE fulfilled=0 AND order_reference=''");
            $db->close();
            return $res->num_rows;
        }
        ...
        {
            $db = $subscriber->GetDB();
            $res = $db->query("SELECT id FROM roompr");
            $db->close();
            return $res->num_rows;
        }
        ...
        public function GenerateOrder($itList, $user)
        {
            if($this->Order_reference == "")
            {
                ...
                while(($db->query("SELECT order_reference FROM roompr WHERE order_reference='$orderRef'")->num_rows > 0))
                {
                    $orderRef = "po-".Random::GenerateId(16);
                }
        ...
        public static function ByReference(Subscriber $subscriber, $reference)
        {
            ...
            $res = $db->query("SELECT * FROM roompr WHERE order_reference='$reference'");
            if($res->num_rows > 0)
            {
                $row = $res->fetch_assoc();

                $ret = new Roompr($subscriber);
                $ret->Id = $row['roomprid'];
