	...
	class Storepo
	{
        ...
        public function Initialize($arg=null)
		{
			if($arg != null)
			{
				...
				$res = $db->query("SELECT * FROM storepo WHERE storepoid='$arg'");

				if($res->num_rows > 0)
				{
					$row = $res->fetch_assoc();
				
					$this->Id = $row['storepoid'];
                    ...
                    for($i = 0; $i < count($it); $i++)
                    {
                        ...
                        $tm = $itm->Item;
                        $itm->Item = new Storeitem($this->subscriber);
		...
		public function Save()
		{
			...
			if($res = $db->query("SELECT storepoid FROM storepo WHERE storepoid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE storepo SET supplier='$supplier',items='$items',user='$user',receiver='$receiver',total='$total',paidamount='$paidamount',note='$note',paid='$paid',received='$received',pr='$pr',receivedate='$receivedate',paydate='$paydate',creditnote='$creditnote' WHERE storepoid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT storepoid FROM storepo WHERE storepoid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO storepo(storepoid,created,supplier,items,user,receiver,total,paidamount,note,paid,received,pr,receivedate,paydate,creditnote) VALUES ('$id','$created','$supplier','$items','$user','$receiver','$total','$paidamount','$note','$paid','$received','$pr','$receivedate','$paydate','$creditnote')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM storepo WHERE storepoid='$id'");
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM storepo WHERE supplier LIKE '%$term%' OR items LIKE '%$term%' OR user LIKE '%$term%' OR receiver LIKE '%$term%' OR total LIKE '%$term%' OR paidamount LIKE '%$term%' OR note LIKE '%$term%' OR paid LIKE '%$term%' OR received LIKE '%$term%' OR pr LIKE '%$term%' OR receivedate LIKE '%$term%' OR paydate LIKE '%$term%'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Storepo($subscriber);
                $ret[$i]->Id = $row['storepoid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='storepoid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM storepo WHERE ".$field." ='$term'");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Storepo($subscriber);
                $ret[$i]->Id = $row['storepoid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM storepo ORDER BY ".$field." ".$order."");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Storepo($subscriber);
                $ret[$i]->Id = $row['storepoid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM storepo");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Storepo($subscriber);
				$ret[$i]->Id = $row['storepoid'];
        ...
        public static function ByReference(Subscriber $subscriber, $refrence)
        {
            ...
            $res = $db->query("SELECT * FROM storepo WHERE pr='$refrence'");
            while(($row = $res->fetch_assoc()) != null)
            {
                $ret[$i] = new Storepo($subscriber);
                $ret[$i]->Id = $row['storepoid'];
                ...
                for($j = 0; $j < count($it); $j++)
                {
                    ...
                    $tm = $itm->Item;
                    $itm->Item = new Storeitem($subscriber);
                    $itm->Item->Initialize($tm);
