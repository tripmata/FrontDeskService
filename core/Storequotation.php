<?php
	...
	// Feb, 13/2020
	// Building class for Storequotation
	*/

	class Storequotation
	{
		public $Id = "";
		public $Created = 0;
		...
		public function Initialize($arg=null)
        {
            if($arg != null)
            {
                ...
                $res = $db->query("SELECT * FROM storequotaion WHERE storequotaionid='$arg'");

                if($res->num_rows > 0)
                {
                    $row = $res->fetch_assoc();

                    $this->Id = $row['storequotaionid'];
                    ...
                    for($i = 0; $i < count($itm); $i++)
                    {
                        ...
                        $this->Items[$i]->Item = new Storeitem($this->subscriber);
		...
		public function Save()
		{
            ...
            if($res = $db->query("SELECT storequotaionid FROM storequotaion WHERE storequotaionid='$id'")->num_rows > 0)
			{
				$db->query("UPDATE storequotaion SET displayreference='$displayreference',sms='$sms',email='$email',associatedsuppliers='$associatedsuppliers',responded='$responded',responsecomplete='$responsecomplete',note='$note',suppliers='$suppliers',items='$items' WHERE storequotaionid = '$id'");
			}
			else
			{
				redo: ;
				$id = Random::GenerateId(16);
				if($db->query("SELECT storequotaionid FROM storequotaion WHERE storequotaionid='$id'")->num_rows > 0)
				{
					goto redo;
				}
				$this->Id = $id;
				$db->query("INSERT INTO storequotaion(storequotaionid,created,displayreference,sms,email,associatedsuppliers,responded,responsecomplete,note,suppliers,items) VALUES ('$id','$created','$displayreference','$sms','$email','$associatedsuppliers','$responded','$responsecomplete','$note','$suppliers','$items')");
			}
		}
		...
		public function Delete()
		{
			...
			$id = $this->Id;
			$db->query("DELETE FROM storequotaion WHERE storequotaionid='$id'");
		}
		...
		public static function Search(Subscriber $subscriber, $term='')
		{
			...
			$res = $db->query("SELECT * FROM storequotaion WHERE displayreference LIKE '%$term%' OR sms LIKE '%$term%' OR email LIKE '%$term%' OR associatedsuppliers LIKE '%$term%' OR responded LIKE '%$term%' OR responsecomplete LIKE '%$term%' OR note LIKE '%$term%' OR suppliers LIKE '%$term%' OR items LIKE '%$term%' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Storequotation($subscriber);
                $ret[$i]->Id = $row['storequotaionid'];
		...
		public static function Filter(Subscriber $subscriber, $term='', $field='storequotaionid')
		{
			$db = $subscriber->GetDB();
			$ret = array();
			$i = 0;

			$res = $db->query("SELECT * FROM storequotaion WHERE ".$field." ='$term' ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Storequotation($subscriber);
                $ret[$i]->Id = $row['storequotaionid'];
		...
		public static function Order(Subscriber $subscriber, $field='id', $order='DESC')
		{
			...
			$res = $db->query("SELECT * FROM storequotaion ORDER BY ".$field." ".$order." ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
                $ret[$i] = new Storequotation($subscriber);
                $ret[$i]->Id = $row['storequotaionid'];
		...
		public static function All(Subscriber $subscriber)
		{
			...
			$res = $db->query("SELECT * FROM storequotaion ORDER BY id DESC");
			while(($row = $res->fetch_assoc()) != null)
			{
				$ret[$i] = new Storequotation($subscriber);
				$ret[$i]->Id = $row['storequotaionid'];
